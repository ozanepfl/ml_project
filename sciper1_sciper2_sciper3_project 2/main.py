...
def optimal_hidden_units(xtrain, xtest, ytrain, y_test):
    for hu in hidden_unit_list:
        parameter_comparison_mlp(hu, [0], 1e-5, 10, 64, xtrain, xtest, ytrain, y_test)

def optimal_activation(xtrain, xtest, ytrain, y_test):
    for ac in activation_list1:
        parameter_comparison_mlp([128], ac, 1e-5, 10, 64, xtrain, xtest, ytrain, y_test)
    for ac in activation_list2:
        parameter_comparison_mlp([128, 128], ac, 1e-5, 10, 64, xtrain, xtest, ytrain, y_test)
    for ac in activation_list3:
        parameter_comparison_mlp([128, 128, 128], ac, 1e-5, 10, 64, xtrain, xtest, ytrain, y_test)                
    
def optimal_learning_rate(xtrain, xtest, ytrain, y_test):
    for lr in lr_list:
        parameter_comparison_mlp([128, 128], [0], lr, 10, 64, xtrain, xtest, ytrain, y_test)

def optimal_maxiter(xtrain, xtest, ytrain, y_test):
    for mi in maxiter_list:
        parameter_comparison_mlp([128, 128], [0], 1e-5, mi, 64, xtrain, xtest, ytrain, y_test)

def optimal_batch_size(xtrain, xtest, ytrain, y_test):
    for b in batch_list:
        parameter_comparison_mlp([128, 128], [0], 1e-5, 10, b, xtrain, xtest, ytrain, y_test)
...
elif __name__ == '__mlpopt__':
    mlp_optimization_run()